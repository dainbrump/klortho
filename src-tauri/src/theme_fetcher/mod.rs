...
fn read_kdeconfig(
    ...
    let output = Command::new(config_reader)
        .arg("--file")
        .arg("kdeglobals")
        .arg("--group")
        .arg(group)
    ...
    if !output.status.success() {
        return Err(format!(
            "Failed to read group {} and key {} from KDE config.",
            group, key
        )
        .into());
    }
...
pub fn get_kde_theme() -> Result<String, Box<dyn std::error::Error>> {
    ...
    cssvar_to_kdecfg.insert(
        "background".to_string(),
        "Colors:Window.BackgroundNormal".to_string(),
    );
    cssvar_to_kdecfg.insert(
        "foreground".to_string(),
        "Colors:Window.ForegroundNormal".to_string(),
    );
    cssvar_to_kdecfg.insert(
        "card".to_string(),
        "Colors:Window.BackgroundNormal".to_string(),
    ...
    cssvar_to_kdecfg.insert(
        "popover-foreground".to_string(),
        "Colors:Window.ForegroundNormal".to_string(),
    );
    cssvar_to_kdecfg.insert(
        "primary".to_string(),
        "Colors:Window.BackgroundAlternate".to_string(),
    );
    cssvar_to_kdecfg.insert(
        "primary-foreground".to_string(),
        "Colors:Window.ForegroundLink".to_string(),
    );
    cssvar_to_kdecfg.insert(
        "secondary".to_string(),
        "Colors:Window.BackgroundAlternate".to_string(),
    ...
    cssvar_to_kdecfg.insert(
        "secondary-foreground".to_string(),
        "Colors:Window.DecorationFocus".to_string(),
    );
    cssvar_to_kdecfg.insert(
        "muted".to_string(),
        "Colors:Window.BackgroundAlternate".to_string(),
    ...
    cssvar_to_kdecfg.insert(
        ...
        "Colors:Window.ForegroundInactive".to_string(),
    );
    cssvar_to_kdecfg.insert(
        "accent".to_string(),
        "Colors:Window.BackgroundAlternate".to_string(),
    ...
    cssvar_to_kdecfg.insert(
        ...
        "Colors:Window.ForegroundNeutral".to_string(),
    );
    cssvar_to_kdecfg.insert(
    ...
    cssvar_to_kdecfg.insert(
        ...
        "Colors:Window.ForegroundNegative".to_string(),
    );
    cssvar_to_kdecfg.insert(
        "border".to_string(),
        "Colors:Button.DecorationFocus".to_string(),
    );
    cssvar_to_kdecfg.insert(
        "input".to_string(),
        "Colors:Window.BackgroundAlternate".to_string(),
        ...
        if cfgtree.len() == 2 {
            match read_kdeconfig(kreadconfig, cfgtree[0], cfgtree[1]) {
                Ok(setting) => {
                    ...
                    if parameters.len() == 3 {
                        match (
                            parameters[0].parse::<i32>(),
                            parameters[1].parse::<i32>(),
                            parameters[2].parse::<i32>(),
                        ) {
                            (Ok(r), Ok(g), Ok(b)) => {
                                let rgb =
                            ...
                            _ => {
                                return Err(
                                    (format!("Error parsing '{}' as RGB values", kde_key)).into()
                                );
                            }
                        }
                    } else {
